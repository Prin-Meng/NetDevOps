...
from smtp_practice.smtp_new_config_send import prin_smtp
        ...
        for email_no in range(msg_count):  # 逐个读取邮件range(10) = 0 - 9
            ...
            for header_name, header_content in part_list[0].items():
                if header_name == 'Subject':
                    ...
                    # 如果主题为cmd:命令，Python则返回这个命令执行的结果
                    cmd = re.fullmatch('cmd:(.+)', mail_dict[header_name]).groups()[0]
                    if cmd:
                        return_info = os.popen(cmd).read()
                        receiver_mail = re.findall('<(.+@.+)>', mail_dict['From'])[0]
                        prin_smtp(mailserver,
                                  # 登陆邮箱
                                  mailuser,
                                  mailpasswd,
                                  # 发送信件的邮箱需要和登陆邮箱相同
...
if __name__ == '__main__':
    ...
    i = 1
    for x in qyt_rec_mail('pop.qq.com', '1276074188@qq.com', 'qwkdxotdggssbagd', save_file=True, delete_email=True):
        print('=' * 50, '第', i, '封信', '=' * 50)
        pprint(x)
        i += 1
